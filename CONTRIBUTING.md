...
These contributions can be made in the form of Issues or [Pull Requests](http://help.github.com/send-pull-requests/) on the [Api repository](https://github.com/ericsizemore/api).

Here are some examples how you can contribute:

* [Report a bug](https://github.com/ericsizemore/api/issues/new?labels=bug,unverified&template=1-bug_report.yml)
* [Propose a new feature](https://github.com/ericsizemore/api/issues/new?labels=enhancement,unverified&template=2-feature_request.yml)
* [Send a pull request](https://github.com/ericsizemore/api/pulls)
...
When you submit code changes, your submissions are understood to be under the same [MIT License](https://github.com/ericsizemore/api/blob/main/LICENSE.md) that covers the project. By contributing to this project, you agree that your contributions will be licensed under its MIT License.
...
- Create your branch from `main` if you plan to implement new functionality or change existing code significantly.
  ...
  - Pull requests for bug fixes must be made for the oldest branch that is [supported](https://github.com/ericsizemore/api/blob/main/SECURITY.md).
...
[Backwards compatibility](https://github.com/ericsizemore/api/blob/main/backward-compatibility.md) breaks in this project are being kept to an absolute minimum. Please take this into account when proposing changes.
...
The following commands can be used to perform the initial checkout of Api:

```bash
$ git clone https://github.com/ericsizemore/api.git
$ cd api
```

Install Api's dependencies using [Composer](https://getcomposer.org/):
...
The configuration file can be found at `phpunit.xml`, in the repository, for [PHPUnit](https://phpunit.de/index.html).

The tests make use of a mock server to run against, which requires [mocko/cli](https://mocko.dev/docs/getting-started/standalone/). To run the tests:
